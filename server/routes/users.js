...
//deposit
router.post('/deposit', auth.authenticate, (req, res) => {
  var input = req.body;
  var amount = parseInt(input.amount);
  var balance = 0;
  var date = moment().format('lll')
  var transaction = `Credited ${amount} on ${date}`;
  userModel.findOne({ _id: req.user_id })
    .then((user_data) => {
      balance = user_data.acc_balance + amount;
      let t_history = user_data.transaction_history
      t_history.unshift(transaction)
      userModel.findOneAndUpdate({ _id: req.user_id }, { acc_balance: balance, transaction_history: t_history })
        .catch((err) => {
          console.log("Error at Updation:", err)
          res.json({ status: 0, Msg: "Deposit failed" })
        })
        .then(() => { console.log("Balance:", balance); res.json({ status: 1, Msg: "Deposit Success", data: { balance: balance } }); })
    })
    .catch(err => { console.log("User not found", err); res.json({ status: 0, Msg: "User not found" }); })
})
//withdraw
router.post('/withdraw', auth.authenticate, (req, res) => {
  var input = req.body;
  var amount = parseInt(input.amount);
  var balance = 0;
  userModel.findOne({ _id: req.user_id })
    .then((user_data) => {
      if (amount > user_data.acc_balance) {
        res.json({ status: 0, Msg: "Insuffient balance" });
        // throw Error("Insuffient balance")
      } else {
        let t_history = user_data.transaction_history
        balance = user_data.acc_balance - amount;
        let date = moment().format('lll')
        let transaction = `Debited ${amount} on ${date}`;
        t_history.unshift(transaction)
        userModel.findOneAndUpdate({ _id: req.user_id }, { acc_balance: balance, transaction_history: t_history })
          .catch((err) => {
            console.log("Error at Updation:", err)
            res.json({ status: 0, Msg: "Withdraw failed" })
          })
          .then(() => { console.log("Balance:", balance); res.json({ status: 1, Msg: "Withdraw Success", data: { balance: balance } }); })
      }
    })
    .catch(err => {
...
//balance
router.post('/balance', auth.authenticate, (req, res) => {
  userModel.findOne({ _id: req.user_id })
    .then((user_data) => {
      res.json({ status: 1, Msg: "Balance retrieved", data: { balance: user_data.acc_balance } });
    })
    .catch((err) => {
      res.json({ status: 0, Msg: "Couldn't retrieve the balance" });
    })
})
//compound interest
router.post('/addInterest', auth.authenticate, (req, res) => {
  var date = moment().format('lll')
  
  userModel.findOne({ _id: req.user_id })
  .then((user_data) => {
    let interest = calC.simpleInterest(parseFloat(user_data.acc_balance));
    let balance = parseFloat(user_data.acc_balance) + parseFloat(interest);
    let newBalance = Number(balance.toFixed(3))
    let transaction = `Credited ${interest} on ${date}`;
    let t_history = user_data.transaction_history
      t_history.unshift(transaction)
      userModel.findOneAndUpdate({ _id: req.user_id }, { acc_balance: newBalance, transaction_history: t_history })
      .catch((err) => {
        console.log("Error at Updation:", err)
        res.json({ status: 0, Msg: "Deposit failed" })
      })
      .then(() => { console.log("Balance:", newBalance); res.json({ status: 1, Msg: "Interest added", data: { balance: newBalance } }); })
  })
  .catch(err => { console.log("User not found", err); res.json({ status: 0, Msg: "User not found" }); })
